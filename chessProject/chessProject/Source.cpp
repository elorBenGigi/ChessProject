...
void main()
{
	...
	while (msgFromGraphics != "quit")
	{
		...
		// extract the source and destination of the coords from the frontend 
...
Coord parseToCoord(std::string move)
{
	...
	{
		throw std::exception("faggot, you gave me an invalid string (unexpected input)");
	}
	// translate the notation to a number and translate the number string to a normal string
	return Coord(move[1] - '0' - 1, NotationToNum(move[0])); // -1 to not get memory access violation error

}

int NotationToNum(char notation)
{
	if (notation < FIRST_NOTATION_EXPECTED || notation > LAST_NOTATION_EXPECTEED)
	{
		throw std::exception("BROTHER IS PLAYING CHECKERS (wrong notation input)");
	}
	return notation - FIRST_NOTATION_EXPECTED;
}